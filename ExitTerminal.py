        ...
        while self.queue.qsize():
            try:
                ...
                if msg == 1:
                    ...
                    self.line5['text'] = "ΕΙΣΑΓΕΤΕ ΤΗΝ ΚΑΡΤΑ ΣΑΣ"
                    self.line6['text'] = ""
                    ...
                    self.line3['text'] = "ΤΟ ΟΧΗΜΑ ΕΧΕΙ"
                    self.line4['text'] = "ΗΔΗ ΕΞΕΛθΕΙ"
                    self.line5['text'] = ""
                    self.line6['text'] = ""
                    # self.master2.after(4000, ThreadedClient.periodicCall(self))
                    ...
                    self.line3['text'] = "ΠΡΕΠΕΙ ΝΑ ΠΛΗΡΩΣΕΤΕ"
                    self.line4['text'] = "ΠΡΙΝ ΤΗΝ ΕΞΟΔΟ"
                    self.line5['text'] = ""
                    self.line6['text'] = ""
                    # self.master2.after(4000, ThreadedClient.periodicCall(self))
                    # threading.Timer(4, ThreadedClient.periodicCall).start()
                    ...
                    self.line3['text'] = "ΠΑΡΑΚΑΛΩ"
                    self.line4['text'] = "ΞΑΝΑΠΡΟΣΠΑΘΗΣΤΕ"
                    self.line5['text'] = ""
                    self.line6['text'] = ""
                    # self.master2.after(4000, ThreadedClient.periodicCall(self))
                    # threading.Timer(4, ThreadedClient.periodicCall).start()
                    ...
                    self.line3['text'] = "ΠΑΡΑΚΑΛΩ ΠΑΡΑΛΑΒΕΤΕ"
                    self.line4['text'] = "ΤΟ ΕΙΣΙΤΗΡΙΟ ΣΑΣ"
                    self.line5['text'] = ""
                    self.line6['text'] = ""
                    # self.master2.after(4000, ThreadedClient.periodicCall(self))
                    # threading.Timer(4, ThreadedClient.periodicCall).start()
                    ...
                    self.line3['text'] = "ΠΑΡΑΚΑΛΩ ΕΠΙΚΟΙΝΩΝΗΣΤΕ"
                    self.line4['text'] = "ΜΕ ΤΟ ΤΑΜΕΙΟ"
                    self.line5['text'] = ""
                    self.line6['text'] = ""
                    # self.master2.after(4000, ThreadedClient.periodicCall(self))
                    # threading.Timer(4, ThreadedClient.periodicCall).start()
...
class ThreadedClient:
    ...
    def workerThreadRFID(self):
        ...
        while self.running:
            ...
            time.sleep(0.5)
            try:
                entries, result, uid = self.rfidblock.readBlockDataExit()
                if len(str(uid)) > 4:
                    buzzer = GeneralOutput()
                    #result = self.rfidblock.readBlockData()
                    print(result)
                    if result == 1:
                        res = self.rfidblock.writeNewBlockDataExit(entries)
                        ...
                        if res == None:
                            ...
                            msg = 201
                            #self.httpreq.receive_card_exit(msg)
                    else:
                        logger.info("Vechicle already exited or was never inside")
                        msg = 503
                    self.httpreq.receive_card_exit(msg)
                    msg_screen = msg
                    #self.queue.put(msg)
